-- Script pour corriger le trigger qui cause l'erreur
-- À exécuter dans le SQL Editor de Supabase Dashboard

-- 1. D'abord, vérifier quel trigger existe
SELECT tgname FROM pg_trigger WHERE tgrelid = 'auth.users'::regclass;

-- 2. Supprimer TOUS les triggers sur auth.users qui pourraient causer le problème
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS handle_new_user ON auth.users;

-- 2. Créer une version simplifiée du trigger sans la gestion des groupes
...
-- 4. Ajouter les utilisateurs existants au groupe public après coup
DO $$
DECLARE
...
BEGIN
  ...
  -- Ajouter tous les utilisateurs au groupe public
