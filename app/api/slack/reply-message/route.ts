...
export async function POST(request: Request) {
  try {
    ...
    const { channel, text, thread_ts } = await request.json()
    
    if (!channel || !text || !thread_ts) {
      return NextResponse.json({ error: 'Canal, message et thread_ts requis' }, { status: 400 })
    }
    ...
    // Create reply message with blocks for better formatting control
    const result = await slack.chat.postMessage({
      channel,
      thread_ts, // Répond dans le thread du message
      blocks: [
        {
          type: 'context',
          elements: [
    ...
    if (group) {
      ...
      await broadcastChannel.send({
        ...
        payload: {
          ...
          thread_ts: thread_ts // Inclure le thread_ts pour indiquer que c'est une réponse
        }
      })
    ...
    if (error.code === 'slack_webapi_platform_error' && error.data?.error === 'missing_scope') {
      return NextResponse.json({ 
        error: 'Le bot Slack n\'a pas les permissions nécessaires. Vérifiez les scopes: chat:write' 
      }, { status: 403 })
    }
    ...
    if (error.code === 'slack_webapi_platform_error' && error.data?.error === 'thread_not_found') {
      return NextResponse.json({ 
        error: 'Message parent introuvable' 
      }, { status: 404 })
    }
